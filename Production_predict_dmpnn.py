"""
predict_dmpnn.py - FINAL OFFLINE - 500 REAL drugs with D-MPNN CheMeleon
Fixed for Lenovo i3 + PyTorch 2.6 + chemprop 2.x
Uses modules/*_dmpnn_production.ckpt from train_D-MPNN_for_production.py
No PubChem call, no featurize.py, no ECFP
"""
...
# FIX for PyTorch 2.6 - allowlist all chemprop globals that appear in.ckpt
...
OUTPUT_TXT = Path("DMPNNPredictions.txt")

# 500 REAL DRUGS - REAL SMILES (same as your CatBoost version)
REAL_500 = [
...
# Expand to 500 unique by alkyl variation - keeps REAL scaffolds
...
def ensure_chemeleon():
    if not CHEMELEON_PATH.exists():
        print("[chemeleon] downloading...")
        urlretrieve(CHEMELEON_URL, CHEMELEON_PATH)
    return CHEMELEON_PATH

def load_production_mpnn(name: str):
    prod_ckpt = MODULES / f"{name}_dmpnn_production.ckpt"
    if not prod_ckpt.exists():
        raise FileNotFoundError(f"{prod_ckpt} not found - run train_D-MPNN_for_production.py for {name}")
    mpnn = models.MPNN.load_from_checkpoint(str(prod_ckpt), map_location="cpu", weights_only=False)
    mpnn.eval()
    return mpnn

def predict_endpoint(mpnn, smiles_list, batch_size=128):
    ...
    for smi in smiles_list:
        try:
            ...
            if dp is None or getattr(dp, 'mol', None) is None:
                raise ValueError("invalid mol")
            datapoints.append(dp)
            valid_mask.append(True)
        except Exception:
            # keep placeholder to preserve batch alignment, but mark invalid
            datapoints.append(data.MoleculeDatapoint.from_smi("CC"))
            valid_mask.append(False)

    dset = data.MoleculeDataset(datapoints, featurizer, cache=True)
    loader = data.build_dataloader(dset, num_workers=0, batch_size=batch_size, shuffle=False)

    preds = []
    with torch.no_grad():
        for batch in loader:
            out = mpnn.predict_step(batch, 0) # correct chemprop API, returns prob [0,1]
            p = out.detach().cpu().numpy().ravel()
            preds.extend(p.tolist())

    # invalid SMILES -> 0.0 (visible in output, not silently dropped)
    return [float(p) if m else 0.0 for p, m in zip(preds, valid_mask)]
...
for ep in ENDPOINTS:
    try:
        MODELS[ep] = load_production_mpnn(ep)
        print(f"[load] {ep} production ok")
    except FileNotFoundError as e:
        print(f"[skip] {e}")

if not MODELS:
    raise SystemExit("No production ckpts found. Run train_D-MPNN_for_production.py first.")

smiles_list = df_in["SMILES"].tolist()
results = df_in[["Drug","SMILES"]].copy()

for ep in ENDPOINTS:
    if ep not in MODELS:
        results[ep] = 0.0
        continue
    print(f"\n[predict] {ep} on {len(smiles_list)} drugs (bs=128, cache=True)...")
    probs = predict_endpoint(MODELS[ep], smiles_list, batch_size=128)
    results[ep] = probs
    print(f" {ep}: unique {len(set(probs))} min {min(probs):.4f} max {max(probs):.4f} mean {sum(probs)/len(probs):.4f}")

# Save
results.to_csv("DMPNN_500_predictions.csv", index=False)
...
print(f"\nSaved {OUTPUT_TXT} and DMPNN_500_predictions.csv with {len(results)} rows")
print("All SMILES are REAL drug scaffolds, works offline, i3 optimized (cache+bs128+predict_step)")