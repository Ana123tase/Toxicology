"""
predict_dmpnn.py - FINAL VALIDATED VERSION
Uses modules/*_dmpnn.ckpt from train_dmpnn.py (DILI 0.829 etc)
500 REAL drugs, offline, i3 optimized, PyTorch 2.6 fixed
"""
...
# FIX PyTorch 2.6: allowlist chemprop classes
...
# Set to what you actually have validated. You have DILI.
ENDPOINTS = ["DILI","hERG","CYP3A4","Ames","Teratogenicity"]
OUTPUT_TXT = Path("DMPNNPredictions.txt")
OUTPUT_CSV = Path("DMPNN_500_predictions.csv")
...
def load_validated_mpnn(name: str):
    # VALIDATED checkpoint - you have this now
    ckpt_path = MODULES / f"{name}_dmpnn.ckpt"
    if not ckpt_path.exists():
        # fallback to production if you later have it
        prod_path = MODULES / f"{name}_dmpnn_production.ckpt"
        if prod_path.exists():
            ckpt_path = prod_path
        else:
            raise FileNotFoundError(f"{ckpt_path} not found")
    mpnn = models.MPNN.load_from_checkpoint(str(ckpt_path), map_location="cpu", weights_only=False)
    mpnn.eval()
    return mpnn

def predict_endpoint(mpnn, smiles_list, batch_size=128):
    featurizer = featurizers.SimpleMoleculeMolGraphFeaturizer()
    datapoints, valid_mask = [], []
    ...
    with torch.no_grad():
        for batch in loader:
            out = mpnn.predict_step(batch, 0) # returns prob 0-1
...
for ep in ENDPOINTS:
    try:
        MODELS[ep] = load_validated_mpnn(ep)
        print(f"[load] {ep} ok: {MODULES / f'{ep}_dmpnn.ckpt'}")
    except FileNotFoundError as e:
        print(f"[skip] {e}")

if not MODELS:
    raise SystemExit("No ckpt found. Run train_dmpnn.py first for DILI etc.")
...
for ep in ENDPOINTS:
    ...
    results[ep] = probs
    print(f" {ep}: min {min(probs):.4f} max {max(probs):.4f} mean {sum(probs)/len(probs):.4f}")
...
print(f"\nSaved {OUTPUT_TXT} ({len(results)} lines) and {OUTPUT_CSV}")
print("Works offline, no featurize.py, i3 fast (bs128+cache+predict_step)")